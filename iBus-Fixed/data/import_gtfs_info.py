...
try:
	...
	for rowvalues in reader:
		cursor.execute("""
				...
				VALUES
				(?, ?, ?, ?, ?, ?, ?, ?, ?)
				""", (rowvalues['id'], rowvalues['name'], rowvalues['state'], rowvalues['country'], rowvalues['website'], rowvalues['dbname'], rowvalues['lastupdate'], rowvalues['lastupdate'], rowvalues['local']))
		addLines = addLines + 1
